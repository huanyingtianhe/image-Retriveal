...
function RGBlist = travelFolders(queryImagePath,queryNum,dirName,mode,rows,cloumns)
...
for i=3:num %前两个文件夹保留
    if ~D(i).isdir
        ...
        image = imresize(tempRGB, [384 256]);
        ...
        elseif mode == 4
            set =[wavelet_moments];
        elseif mode == 5
            set = [hsvHist meanAmplitude msEnergy wavelet_moments];
        end
...
function fea = queryFeatureVector(dirName,img,mode)
	...
	elseif mode == 4
        fea =[wavelet_moments];
    elseif mode == 5
        fea = [hsvHist meanAmplitude msEnergy wavelet_moments];
    end
